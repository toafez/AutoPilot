...
function language() {
	...
	if [[ "${1}" == "PILOT" ]]; then
		if [ -f "/var/packages/AutoPilot/target/ui/lang/pilot/lang_pilot_${script_lang}.txt" ]; then
			source "/var/packages/AutoPilot/target/ui/lang/pilot/lang_pilot_${script_lang}.txt"
		fi
	fi
}
